...
for i, scan in enumerate(scan_no):
    ...
    q1, ij_mapping_T = calibrate(folder, calib_folder, 'q1', ij_mapping_T, np.array([reduced_q[x[0]] for x in peak_locs]), x_loc)
    q1int, ij_mapping_T = calibrate(folder, calib_folder, 'q1int', ij_mapping_T, np.array([reduced_intensity[j][x[0]] for j,x in enumerate(peak_locs)]), x_loc)
    ...
    q2int, ij_mapping_T = calibrate(folder, calib_folder, 'q2int', ij_mapping_T, np.array([reduced_intensity[j][x[1]] if len(x) == 2 else np.nan for j,x in enumerate(peak_locs)]), x_loc)
    _, ij_mapping_T = calibrate(folder, calib_folder, 'qratio', ij_mapping_T, q1/q2, x_loc)
    _, ij_mapping_T = calibrate(folder, calib_folder, 'qdist', ij_mapping_T, q1-q2, x_loc)
    _, ij_mapping_T = calibrate(folder, calib_folder, 'iratio', ij_mapping_T, q1int/q2int, x_loc)
    _, ij_mapping_T = calibrate(folder, calib_folder, 'ratio', ij_mapping_T, [(q1int[i]/x[pinned_sl])/(q1[i]/reduced_q[pinned_sl]) for i,x in enumerate(reduced_intensity)], x_loc)
    ...
    for j,_ in enumerate(reduced_intensity):
        ...
        plt.axvline(x=q1[j], linestyle='--', color='C0', label='q1 = ' + str(round(q1[j], 2)))
    ...
    with open(folder + '/' + str(scan) + '_data.pckl', 'wb') as f:
        pickle.dump([x_loc, reduced_q, reduced_intensity, q1, q2, q1int, q2int], f)
