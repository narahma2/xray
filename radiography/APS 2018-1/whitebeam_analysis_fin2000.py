# -*- coding: utf-8 -*-
"""
Created on Tue Aug  6 15:51:04 2019
...
for i, tt in enumerate(tests):        
    ...
    peaks, _ = find_peaks(savgol_filter(data_epl, 105, 7), width=20, prominence=0.04)
...
for i, tt in enumerate(tests):        
    ...
    for z, k in enumerate(cropped_view):
        warnings.filterwarnings("ignore")
        peaks, _ = find_peaks(savgol_filter(data_epl[k, :], 105, 7), width=20, prominence=0.04)
        warnings.filterwarnings("default")
...
#%% Calculate correction factor based on 700 um
...
#%% Apply results to a 2000um, 0% KI case
case = 0
...
processed_data2000 = len(tests) * [None]

loop_start = timer()
for i, tt in enumerate(tests):        
    data2000 = np.array(Image.open(tt))
    warnings.filterwarnings("ignore")
    data_norm2000 = (data2000-dark) / flatfieldavg_darksub
    warnings.filterwarnings("default")
    offset = 1-np.median(data_norm2000[50:300, 25:125])
    
    data_alpha2000 = np.empty(np.shape(data_norm2000), dtype=float)
    data_epl2000 = np.empty(np.shape(data_norm2000), dtype=float)
    cropped_view = np.linspace(start=1, stop=340, num=340, dtype=int)
    left_bound = len(cropped_view) * [np.nan]
    right_bound = len(cropped_view) * [np.nan]
    model_epl2000 = len(cropped_view) * [np.nan]
    optical_diameter2000 = len(cropped_view) * [np.nan]
    axial_position = len(cropped_view) * [np.nan]
    lateral_position = len(cropped_view) * [np.nan]
    fitted_graph2000 = len(cropped_view) * [np.nan]
    epl_graph2000 = len(cropped_view) * [np.nan]
    
    for z, k in enumerate(cropped_view):
        j = np.argmin(abs(k-np.array(angle_to_px)))
        data_alpha2000[k, :] = corrected_water_model[1][j](data_norm2000[k, :] + offset)
#        epl2000 = spray_epl[[np.nanargmin(abs(atten_water[j]-x)) for x in data_alpha2000[k, :]]]
#        epl_ind = [np.nanargmin(abs(x-spray_epl)) for x in epl2000]
#        alpha_KI_corr = data_alpha2000[k, :] - [pfits[j][i].c[0]*(test_KI-0) for i in epl_ind]
#        data_alpha2000[k, :] = alpha_KI_corr
        data_epl2000[k, :] = spray_epl[[np.nanargmin(abs(atten_water[j]-x)) for x in data_alpha2000[k, :]]]
        
    for z, k in enumerate(positions):
        data_epl2000[int(k), :] = data_epl2000[int(k), :] * cf[int(z)]
        
    for z, k in enumerate(cropped_view):
        warnings.filterwarnings("ignore")
        peaks, _ = find_peaks(savgol_filter(data_epl2000[k, :], 105, 7), width=20, prominence=0.1)
        warnings.filterwarnings("default")
        
        if len(peaks) == 1:
            warnings.filterwarnings("ignore")
            [full_width, fifth_max, lpos, rpos] = peak_widths(savgol_filter(data_epl2000[k, :], 105, 7), peaks, rel_height=0.8)
            warnings.filterwarnings("default")
            full_width = full_width[0]
            fifth_max = fifth_max[0]
            left_bound[z] = lpos[0]
            right_bound[z] = rpos[0]
            warnings.filterwarnings("ignore")
            model_epl2000[z], fitted_graph2000[z], epl_graph2000[z] = ideal_ellipse(y=savgol_filter(data_epl2000[k,:], 105, 7)[int(round(lpos[0])):int(round(rpos[0]))], full_width=full_width, relative_max=fifth_max, dx=cm_pix)
            warnings.filterwarnings("default")
            optical_diameter2000[z] = full_width * cm_pix
            axial_position[z] = k
            lateral_position[z] = int(np.mean([lpos[0], rpos[0]]))
    
    if len(peaks) == 1:
        signal = np.nanmean(data_epl2000[10:340, int(round(np.nanmean(lateral_position)))-20:int(round(np.nanmean(lateral_position)))+20])
    else:
        signal = 0
        
    noise = np.nanstd(data_epl2000[50:300, 25:125])
    SNR = signal / noise
    
    processed_data2000[i] = {"Image": tt, "Optical Diameter": optical_diameter2000, "Model EPL Diameter": model_epl2000, "Axial Position": axial_position,
...
f = open("R:/APS 2018-1/Imaging/Processed/Corrected/" + test_name + "_corrected.pckl", "wb")
pickle.dump(processed_data2000, f)
f.close()

processed_data2000 = processed_data2000[:i+1]

#%% CF calculation for the 2000um case after 700um calibration
axial_positions2000 = [x["Axial Position"] for x in processed_data2000]
axial_positions2000 = [x for x in axial_positions2000 for x in x]

lateral_positions2000 = [x["Lateral Position"] for x in processed_data2000]
lateral_positions2000 = [x for x in lateral_positions2000 for x in x]

optical_diameters2000 = [x["Optical Diameter"] for x in processed_data2000]
optical_diameters2000 = [x for x in optical_diameters2000 for x in x]

model_epl2000 = [x["Model EPL Diameter"] for x in processed_data2000]
model_epl2000 = [x for x in model_epl2000 for x in x]

offset = np.mean([x["Offset"] for x in processed_data2000])
SNR = np.mean([x["SNR"] for x in processed_data2000])

summary2000 = pd.DataFrame({"Axial Position": axial_positions2000, "Lateral Position": lateral_positions2000, 
                        "Optical Diameter": optical_diameters2000, "Model EPL Diameter": model_epl2000})

summary2000 = summary2000.sort_values(["Axial Position", "Lateral Position"])
summary2000 = summary2000.reset_index(drop=True)
summary2000_list = [v for k, v in summary2000.groupby("Axial Position")]

positions2000 = [np.mean(x["Axial Position"]) for x in summary2000_list][5:]
optical_diameters2000_avg = [np.mean(x["Optical Diameter"]) for x in summary2000_list]
model_epl2000_avg = [np.mean(x["Model EPL Diameter"]) for x in summary2000_list]
cf2000 = [x1/x2 for x1,x2 in zip(optical_diameters2000_avg, model_epl2000_avg)][5:]
