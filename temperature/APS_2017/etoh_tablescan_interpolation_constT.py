"""
Compares the interpolated IJ Ramping temperatures from the calibration to the
Nozzle T. This script tests how much the temperature changes at the
different positions.
...
def main():
    ...
    # Positions array
    positions = np.loadtxt(glob.glob(folder + '/IJ Ramping/Temperature/T*')[0]
                           + '/positions.txt')
    ...
    for calibration in calibrations:
        ...
        for j, profile in enumerate(profiles):
            ...
            if calibration == 'Combined':
                cmb_fld = folder.replace('APS 2017-2', 'APS 2018-1')\
                          .replace('Ethanol', 'Ethanol_700umNozzle')
                p = np.poly1d(np.loadtxt(cmb_fld + '/' + calibration +
                                         '/Statistics/' + profile +
                                         '_polynomial.txt'))
            else:
                p = np.poly1d(np.loadtxt(folder + '/' + calibration +
                                         '/Statistics/' + profile +
                                         '_polynomial.txt'))

            # Create folders
            plots_folder = folder + '/IJ Ramping/TemperatureInterp/' +\
                           calibration + '_' + profile
            ...
            for i, fld in enumerate(flds):
                ...
                data = np.loadtxt(fld + '/Profiles/profile_' + profile +
                                  '.txt')
            ...
            [
             plt.plot(
                      ...
                      '-o',
                      color='C'+str(i),
                      label=str(round(combined_nozzleT[x])) + ' K'
                      )
